...
###8:40 PM 11/13
###Defenders not being updated accordingly in Offensive_Player.find_defender()
###This is likely what's impacting the danger scores and causing them to be incorrect
###We also do not yet account for orientation.
###Then we can export and start writing functions to get danger ratings for data
###In this new file, we will also implement leave-one-out as a method to check how much a player impacts total danger
###We also need to implement neighboring at one point, maybe as a flag for make_big_circ.
###Also some type of orientation measure - Maybe Using Leverage
###We will likely do so with a similar angle and radius trick, going a little less than 90 degrees from the given orientation in each direction.

###4:22 PM 11/15
##Fixed point 1 an 2
##Still need to account for orientation
##Still need a function, possibly a class, for a mega-circle, the result of all the twisting

##10:24 PM 11/15:
##Mostly Good now. Need to investigate why Final heatmap only has one candidate repeated 4 times in each candidate list.
##Might try and increase the values in the danger formula, so they aren't all decimals around 0.2 or less.

###6:15 PM 11/18/2023
##Wheel works for player imports
##Colors are a little light, need to make them harsher in the end, but for now it's fine
##Still not taking into account player orientation for danger calculation.
##Might try to get an animated wheel in here and then export.

###1:43 PM 11/24/2023:
###updated_find_defender to determine blocking
###Need to update danger function

##12:02 AM 11/25/2023:
##Need to fix leverage, specifically theta 2. Need to determine how far to project player, wheather constant or based on positioning of Defender/OL
##Also still haven't totally updated danger function. It's halfway done.
